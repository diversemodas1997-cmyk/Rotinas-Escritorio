...
function UpdatesPanel({ itemName, updates = [], onAddUpdate, onEditUpdate, onDeleteFile, onClose, allPeople, currentUserName }) {
...
function BoardView({ tasks, setTasks, apiUpdateTask, apiUpdateSub, apiAddTask, apiAddSubitem, search, allPeople, columns, setColumns, apiUpdateColumn, apiDeleteColumn, setShowAddCol, subColumns, setSubColumns, setShowAddSubCol, onOpenUpdates, perms = {} }) {
  ...
  const subExtraCols = subColumns.length > 0 ? " " + subColumns.map(c => c.width || "80px").join(" ") : "";
      ...
      {groupOpen && (
        <div style={{ borderRadius: 8, overflow: "hidden", border: "1px solid #2a2d35" }}>
            ...
            {allCols.map((col, ci) => (
              <div key={col.id}
                ...
                <ColHeader col={col}
                  ...
                  onDelete={perms.deleteColumns ? () => { if (apiDeleteColumn) apiDeleteColumn(col.id); else setColumns(p => p.filter(c => c.id !== col.id)); } : null}
          ...
          {filtered.map((task, ti) => {
            ...
            const subReorder = (newSubs) => setTasks(prev => prev.map(t => t.id === task.id ? { ...t, subitems: newSubs } : t));
    ...
    <div>
      ...
      {task.subitems.map((sub, si) => (
        <div key={sub.id}
          ...
          {subColumns.map(sc => <div key={sc.id} style={cellStyle()}><CellRenderer col={sc} item={sub} onChange={ns => upSub(task.id, sub.id, ns)} allPeople={allPeople} small /></div>)}
          {perms.addColumns ? (
            <div onClick={() => setShowAddSubCol(true)} style={{ ...cellStyle({ borderRight: "none", cursor: "pointer", color: "#579bfc", fontSize: 13, fontWeight: 700, opacity: 0.4 }) }}
...
function Dashboard({ currentUser, onLogout }) {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', system-ui, sans-serif", background: "#13151a", color: "#e8eaed", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      {updatesData && <UpdatesPanel itemName={updatesData.name} updates={updatesData.updates} currentUserName={currentUser.name} onAddUpdate={apiAddUpdate} onEditUpdate={(updateId, newText, newFiles) => {
        if (!updatesTarget) return;
        setTasks(prev => prev.map(t => {
          if (t.id !== updatesTarget.taskId) return t;
          if (updatesTarget.subId) {
